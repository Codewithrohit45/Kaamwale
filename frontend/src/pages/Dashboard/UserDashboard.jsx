...
export default function UserDashboard({ hideHeader = false }) {
  ...
  useEffect(() => {
    ...
    const handleWorkerLocation = ({ bookingId, coords }) => {
    ...
    socket.on("workerLocationChanged", handleWorkerLocation);

    return () => {
      socket.off("workerLocationChanged", handleWorkerLocation);
    };
  }, [socket]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBookings(
          bookings.map((b) =>
            b._id === bookingId
              ? { ...b, status: "cancelled", cancelledBy: "user" }
              : b
          )
        );
  ...
  const handlePayment = async (booking) => {
    ...
    try {
        ...
        handler: async (response) => {
          try {
            const verifyRes = await fetch(
              `http://localhost:5000/api/orders/${orderData.order._id}/payment`,
              {
                method: "PUT",
                headers: {
                  "Content-Type": "application/json",
      ...
      const isMockMode = !orderData.key_id || orderData.key_id.startsWith('rzp_test_placeholder');
      if (isMockMode) {
        const confirmPayment = window.confirm(
          "💳 KAAMWALE PAYMENT SIMULATOR\n\nNo real Razorpay API keys detected in your backend .env file.\n\nWould you like to simulate a successful payment for this booking?"
        );
        if (confirmPayment) {
          const mockResponse = {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setBookings(
          bookings.map((b) =>
            b._id === disputeModal
              ? { ...b, dispute: { isRaised: true, ...disputeData } }
              : b
          )
        );
        ...
        {[
          {
            ...
            icon: <FiClock />,
            color:
              "text-slate-600 bg-slate-100 dark:bg-slate-700/50 dark:text-slate-300",
          },
          {
            label: "Active",
            value: stats.active,
          ...
          <div
            key={i}
            className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 transition-colors"
          >
            <div
          ...
          {["all", "pending", "accepted", "in-progress", "completed", "cancelled"].map(
            (f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-1 md:flex-none px-5 py-3.5 text-sm font-medium capitalize whitespace-nowrap transition-colors ${
                  filter === f
        ...
        <div className="p-6 space-y-4">
          {filteredBookings.length === 0 ? (
            ...
            filteredBookings.map((booking) => (
              <div
                key={booking._id}
                className={`p-5 border rounded-2xl relative overflow-hidden transition-colors ${
                  booking.status === "accepted" || booking.status === "in-progress"
                    ? "border-teal-200 dark:border-teal-900/50 bg-white dark:bg-slate-800"
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <img
                      src={
                        ...
                        `https://ui-avatars.com/api/?name=${booking.provider?.name?.replace(
                          " ",
                          "+"
                        )}&background=random`
                      }
                    ...
                    <div className="min-w-0">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-bold rounded capitalize ${getStatusStyle(
                          booking.status
                        )}`}
                      >
                      ...
                      <Link
                        ...
                        className="block font-bold text-slate-800 dark:text-white hover:underline mt-1 truncate"
                      >
                  ...
                  <div className="flex-1 grid grid-cols-3 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p
                        className="font-semibold text-slate-800 dark:text-white text-xs truncate"
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                    ...
                    {booking.status === "in-progress" && (
                      <button
                        ...
                        className="px-3 py-2 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 font-medium rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/30 transition-colors text-sm flex items-center gap-1"
                      >
                      ...
                      <>
                        {booking.paymentStatus !== "paid" && (
                          ...
                          >
                            ...
                            {paymentLoading === booking._id ? "..." : "Pay Now"}
                          </button>
                        )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-700">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  <option value="Provider was unprofessional">
                    Provider was unprofessional
                  </option>
                  <option value="Incorrect billing/price">
                    Incorrect billing/price
                  </option>
                  <option value="Provider did not show up">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <Button
                variant="primary"
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 border-red-600"
        ...
        <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 w-full max-w-sm shadow-2xl text-center border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in duration-300">
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-6">
              Please share this completion OTP with the worker to finalize the
              payment.
            </p>
        ...
        <div className="fixed inset-0 bg-black/60 z-[70] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-4xl h-[80vh] flex flex-col shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700 animate-in zoom-in duration-300">
            <div className="p-6 border-b dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Tracking: {trackingModal.provider?.name}
                </p>
              </div>
            ...
            <div className="flex-1 relative">
              <MapContainer
                center={[
                  workerLocations[trackingModal._id]?.lat || 19.076,
                  workerLocations[trackingModal._id]?.lng || 72.8777,
                ]}
                zoom={14}
